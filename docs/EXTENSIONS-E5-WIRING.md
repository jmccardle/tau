...
- **S28 — consume threaded tool/prompt flags ✅** (`18e98d5`). `-xt` filters the
  resolved built-ins in `TauBackend` (both paths); `-nbt` → `tools=[]`
  *(**superseded**: S28 claimed this alone made `-nbt` distinct from `--no-tools`,
  but `_build_turn_tools` merged extension tools in unconditionally, so both flags
  still meant the same thing. The distinction is real only since the resolved
  `no_tools` tri-state — `headless.resolve_no_tools`, pi `main.ts:424-428` — which
  `AgentSession` consumes to suppress the merge under `"all"`.)*;
