...
| Provider Registry | `ProviderRegistry` | `tau_llm.providers.registry.ProviderRegistry` |
| Model Types | `Model` interface | `tau_llm.types.Model` (pydantic) |
...
| Package boundaries | τ-llm (providers) → τ-agent-core (agent loop) → τ-coding-agent (TUI) |
...
├── tau-llm/            # AI providers (replaces pi-ai)
│   └── src/tau_llm/
...
```python
from tau_llm.streaming import ErrorEvent
...
| τ README (tau-llm) | `/tau-llm/README.md` |
