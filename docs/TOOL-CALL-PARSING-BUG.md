...
`tau-llm/src/tau_llm/providers/openai.py`, in `stream_chat`'s `event_generator`, lines **680–688**:
...
| File | Lines | Issue |
|---|---|---|
| `tau-llm/.../providers/openai.py` | 680–688 | **root cause** — cumulative-string assumption |
| `tau-llm/.../providers/openai.py` | 660, 665–672 | `index` computed but unused; position-keyed accumulation (parallel-call bug) |
| `tau-llm/.../providers/openai.py` | 472–476, 508–513, 709–711, 393–395 | four copies of the `{"raw": ...}` fallback |
...
1. **`tau-llm/src/tau_llm/json_parse.py`** (new) — Python port of pi's `json-parse.ts`: `repair_json`, `parse_json_with_repair` (strict, raises), `parse_streaming_json` (best-effort partial parse, returns `{}`).
...
`tau-llm/tests/test_tool_call_streaming_fix.py` (new, 13 tests, all passing) drives the **real** provider with a fragmenting SSE feed (the mock implements `aiter_lines`, unlike the older fixtures):
