...
- [ ] Create `tau-llm/` with `pyproject.toml`, `src/tau_llm/`
...
## Phase 1 — τ-llm (OpenAI Provider) (1 week)

**Goal**: A working OpenAI provider that can stream text and tool calls.

### 1.1 Core Types (1 day)
- [ ] `tau_llm/types.py` — All message, tool, model, usage types
- [ ] `tau_llm/tools.py` — `define_tool()`, `validate_tool_arguments()`
- [ ] `tau_llm/streaming.py` — `AssistantMessageEventStream` async iterator

### 1.2 OpenAI Provider (3 days)
- [ ] `tau_llm/providers/base.py` — Abstract `Provider` class
- [ ] `tau_llm/providers/openai.py` — `OpenAICompletionsProvider`
...
- [ ] `tau_llm/providers/registry.py` — Provider registration

### 1.3 Streaming Integration (2 days)
- [ ] `tau_llm/client.py` — `stream_simple()` function (agent loop entry point)
...
### Deliverable:
```python
from tau_llm import stream_simple, Model
...
# register_tool takes a plain dict, NOT a tau_llm.ToolDefinition — its
# `execute` has the five-argument extension signature above. tau_llm's
# define_tool() builds the other tool shape; see docs/extensions.md.
hello_tool = {
    "name": "hello",
    "label": "Hello",
...
- [ ] Remove Anthropic/Gemini backend code (τ-llm only)
...
- [ ] Anthropic provider (add to τ-llm)
- [ ] Google provider (add to τ-llm)
...
| 1. τ-llm | 1 week | OpenAI streaming provider |
...
|------|--------|------------|
| τ-llm provider interface changes during core dev | Medium | Lock types in Phase 1, test early with Phase 2 |
...
### τ-llm: Pydantic vs raw dicts for messages

**Decision**: Use **pydantic models** for τ message types internally, but convert to **raw dicts** at the API boundary (τ-llm → τ-agent-core boundary).
...
- The **parent design docs** (tau-llm.md, tau-agent-core.md, tau-coding-agent.md)
