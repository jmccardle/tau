...
- `--thinking {off|minimal|low|medium|high|xhigh}` and the `--model x:high`
  suffix — both set the requested reasoning level. τ-llm now has the
  `reasoning_effort` send-path: `Model` gained `reasoning`/`thinking_level_map`
  fields, `tau_llm/models.py` ports pi's `clampThinkingLevel`, and
...
`--mode rpc` all thread it through `store_factory.build_session_catalog` into
`FileSessionCatalog(base_dir=DIR)` — seam 1 of docs/SESSION-UX-REDESIGN.md,
which `Session.create`/`fork`/`list_sessions`/`most_recent` already carried.

The **default** is per mode, and that is the point:

| Mode | Default session base |
|---|---|
| TUI, `--print` | `~/.tau/sessions` (unchanged) |
| `--mode rpc` | `<tempdir>/.tau-<uid>/sessions` (i.e. `/tmp/.tau-1000/sessions` unless `$TMPDIR` says otherwise) |

Why RPC differs: `SessionCatalog.create` writes a session header immediately
and unconditionally, so once `--mode rpc` started binding a persisted session
(docs/RPC-TIER-B.md D-6) *every spawn* left a durable, listable, 0-message
session under `~/.tau/sessions/<dashed-cwd>/` — including a child that asks
`get_capabilities` and exits. `--continue` is exactly
`catalog.most_recent(os.getcwd())` (`headless._select_session`), so an editor
plugin spawning τ per request silently took `tau -p -c` away from the human
working in the same repo, and filled the TUI picker with nameless rows.

Consequences worth knowing before relying on either:

- **An explicit `--session-dir` always wins**, in both directions.
  `--mode rpc --session-dir ~/.tau/sessions` is how a host says "yes, I really
  do want these in the user's list"; `tau --session-dir /tmp/.tau-$(id -u)/sessions` is
  how a human reviews what the RPC hosts wrote. It is the one session flag
  `--mode rpc` accepts at startup (`--continue`/`--session`/`--fork`/
  `--resume`/`--name`/`--store` are still rejected there).
- **Durability is bounded by machine uptime** in the RPC default: most systems
  clear the temp dir on reboot. Stated on the wire too, in `set_model`'s and
  `set_session_name`'s `notes`.
- **`<tmp>/.tau-<uid>` is created `0700`, and a pre-existing path that is not
  a directory we own is REFUSED** (`session_store._ensure_private_dir`) — a
  symlink planted there would otherwise redirect transcripts into someone
  else's home. τ never follows it and never silently picks another path. The
  uid is in the name so that two legitimate users on a shared, sticky `/tmp`
  do not contend for one entry: the flat `<tmp>/.tau` this first shipped as
  made `--mode rpc` refuse to start for every user but the first (round-3
  finding 1).
- **File store only.** `--session-dir` with `--store jmfts` raises: a document
  server has no directory, and a flag that would be ignored is refused instead
  (Fail-Early). The *default* above is likewise file-store-only — a
  jmfts-configured RPC run still lands in the same document tree the user's own
  sessions do. Closing that would need a distinct `session_store.parent_id` for
  RPC runs; unclaimed.
- **`--no-session` + `--session-dir` is refused** — nothing is persisted, so
  there is no directory to persist into.
- **`--no-session` alone means the startup session is ephemeral**, in
  `--mode rpc` exactly as in `--print`. For one release it did not: `cli.py`
  parsed the flag, the `--mode rpc` block did not reject it, and
  `rpc_mode.run_rpc` did not read it, so the run persisted anyway. Under the
  flag no session file is written to either base directory, and the appending
  wire verbs (`set_model`, `set_session_name`, `compact`) refuse with `-32004
  SESSION_NOT_PERSISTED` rather than returning a cursor for a write that dies
  with the process. `get_state`'s `addressable` reports the same fact up
  front. The `<tmp>/.tau-<uid>/sessions` base is still resolved and checked at
  startup, because the catalog stays live for a later `new_session {"persist":
  true}` — and because deferring the hostile-squat refusal would move a
  startup safety gate into a mid-run wire failure.
- **`--no-session` does not require the session store to be up**, in either
  mode. Reported by Tectum's prototyping: `tau -p --no-session` exited 2 at
  startup when the configured JMFTS store was unreachable, refusing a run over
  a dependency it does not have. The §3.1 startup health check is now scoped to
  runs that will write (`build_session_catalog(..., persist=False)` →
  `build_jmfts_client(health_check=False)`). Configuration errors still raise;
  only the `GET /` is dropped. Under `--mode rpc` the four store-backed verbs
  stay reachable and fail loudly on their own request instead.
...
`--print`/`-p` (+ message-eating) · `--mode {text,json}` (replacing `--output`) · `--model` with `provider/id:thinking` shorthand · `--provider` (long-only, drop tau's `-p`) · `--continue`/`-c` · `--tools`/`-t` · `--no-tools`/`-nt` · `--thinking` (gated on tau-llm `reasoning_effort` send-path) · `@file` + positional messages · `--help`/`-h` · `--version` (resolve the `-v` collision).
