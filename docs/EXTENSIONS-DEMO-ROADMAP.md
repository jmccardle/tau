...
Status: **SHIPPED** — the entire E6–E11 arc (S38–S75) is merged, through
2026-07-05. Successor to `EXTENSIONS-E5-WIRING.md` (E0–E5 landed through
S37). Step numbering continues at S38. Still open, by written decision below
(§ "M4"/"M5"), not stalled work: M4 (`registerProvider`) and M5 (package
manager) — see `ROADMAP.md`'s "Extensions epic (Tier 11)" entry for the
compressed shipped-state summary. The gap table in §0 below is the record of
what E6–E11 closed, not a current-state list — every row it marks E6–E10 is
now built.
...
Non-goals for this arc: `registerProvider` (provider work is tau-llm's, not an
