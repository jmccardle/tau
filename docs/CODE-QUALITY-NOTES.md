...
> **Status (2026-06-20):** **#1, #2, #3, #4, #6, #8 FIXED** (tool-call work — see `docs/TOOL-CALL-PARSING-BUG.md` → Resolution), then **#9 FIXED** (stray dir `git rm`'d), **#11 FIXED** (streaming-test mocks now feed `aiter_lines`), and **#5 FIXED** (no fabricated key; raise on missing; key threaded end-to-end — see below). Those previously-failing 24 `test_agent_session`/`test_phase6_subphase3_errors` tests were the silent dependence on a live key #5 created; they now run offline via a `fake_llm` fixture. Full suite now **1360 passed / 0 failed**. Still open: **#7** (the provider's build methods still duplicate the join/parse loop), **#10** (prompt dedup).
