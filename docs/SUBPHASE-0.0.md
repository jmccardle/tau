...
**Reference**: `MONOREPO-STRUCTURE.md`, lines 26-32; `docs/tau-llm.md`, lines 1-40; `docs/tau-agent-core.md`, lines 1-60.
...
### 2. Tool Definitions (tau-llm + tau-agent-core)

```python
# Phase 1 (tau-llm) defines the raw tool definition:
ToolDefinition = {
...
**Reference**: `docs/tau-llm.md`, lines 140-180; `docs/tau-agent-core.md`, lines 120-160.
...
**Reference**: `docs/tau-llm.md`, lines 80-140; `docs/tau-agent-core.md`, lines 160-220.
...
### τ-llm → τ-agent-core boundary

τ-agent-core imports:
- `tau_llm.types.*` — all message, tool, model types
- `tau_llm.tools.*` — `define_tool()`, `validate_tool_arguments()`
- `tau_llm.client.*` — `stream_simple()`

τ-agent-core does NOT import:
- `tau_llm.providers.*` — the agent loop doesn't care about providers
- `tau_llm.streaming.*` — only uses `stream_simple()` return value
...
An extension is a module exposing `register(api)`. It normally imports **nothing
from τ at all**: `api` and `ctx` arrive as arguments and are used duck-typed, and
a tool is a plain dict handed to `api.register_tool`. Of the 42 files under
`examples/`, 34 import no τ package at run time. All eight that do are listed
below; four of them are the `1x_sdk_*.py` SDK scripts, which are not extensions.

Extensions do NOT import:
- `tau_agent_core.agent_loop`
- `tau_agent_core.session_manager`
- `tau_agent_core.extensions.*` — the registry and runner are what `api` fronts

Extensions MAY import, when they need a value type rather than a service:
- `tau_llm.AbortSignal` — to hold or check a cancellation token
  (`examples/51_delegate_fleet.py`)
- `tau_llm.constraints` — `DecodeConstraints` / `ConstraintViolation` for a
  constrained call (`examples/60_retrieval_review.py`)
- `tau_agent_core.tools` — the tool types, for a helper that shadows a built-in
  by name (`examples/ext_kit/steer.py`'s `wrap_tool`)
- `tau_coding_agent.*` — TUI-only, and therefore only for an extension that has
  already given up running headless (`examples/70_telemetry.py` imports
  `chat_widgets.format_telemetry`)
- `tau_agent_core.ExtensionAPI` / `ExtensionContext` — for type annotations only;
  no shipped example needs them, since `api: Any` is enough

An earlier version of this section listed `tau_agent_core.define_tool` as an
extension import and barred `tau_llm.*` outright. Both were wrong: that name has
never existed in `tau_agent_core` (`define_tool` is `tau_llm`'s, and builds the
provider-layer `ToolDefinition`, whose `execute` takes the tool's own arguments —
NOT the five-argument shape `register_tool` wants), and two shipped examples
import `tau_llm` value types today.
...
The key insight: **τ-coding-agent never calls τ-llm directly**. All LLM interaction flows through `AgentSession` → `AgentLoop` → `stream_simple()`. This keeps the TUI decoupled from provider implementation details.
