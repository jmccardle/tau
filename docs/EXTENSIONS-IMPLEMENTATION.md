...
- **A cost field already exists, always empty.** `Usage.cost: dict` is defined but never
  populated (`tau-llm/.../types.py:84`); token usage is real end-to-end
...
behind E1 (§7 decision E0-d). **Superseded:** the degeneracy outlived E1 (the
`_build_turn_tools` merge ignored *why* the tool list was empty) and is closed by a
single resolved `no_tools` tri-state — `"all"`/`"builtin"`/`None`, collapsed at the
argv boundary in `headless.resolve_no_tools` (pi `main.ts:424-428`) and consumed by
`AgentSession._build_turn_tools`. Deprioritize `--session-id`/`--session-dir` (Tier 7).
...
E0-d ship `-nbt` degenerate-to-`--no-tools` with a doc note (**superseded** — see §E0.2). E1-a mutating hooks are a
