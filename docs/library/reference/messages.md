...
```python
is_displayed(message: dict[str, Any]) -> bool
```

`tau_agent_core.messages.is_displayed`
...
- `message: dict[str, Any]` — A stored message dict. Any role; only ``custom`` carries the key.

**Returns**

``False`` only for a ``custom`` message whose ``display`` is literally ``False``. A message with no key, or any other role, is displayed — an older node predates the key and never meant to be hidden.
