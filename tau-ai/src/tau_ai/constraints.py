...
class DecodeConstraints(BaseModel):
    ...
    # tau_ai.grammar.Grammar (a str subclass carrying its own output checker) down to a
        ...
        if self.grammar is not None and self.verify is None:
            ...
            if checker is None:
                raise ValueError(
                    ...
                    "answer.\n"
                    "Either build the grammar with tau_ai.grammar (choice/fixed/regex/"
    ...
    def verify_output(self, text: str) -> None:
        ...
        **τ never trusts a constraint blindly.** The failure mode is real and observed
        ...
        # (a tau_ai.grammar Grammar) or with verify= set (handled above) — so there is
