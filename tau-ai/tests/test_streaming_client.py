"""The streaming client boundary — ``tau_ai.client.stream_simple`` and
``tau_ai.streaming.AssistantMessageEventStream``.
...
Coverage (this file alone, `--cov=tau_ai`): streaming.py 81% -> 99% (only the
...
def _client_factory(response: "_FakeResponse"):
    ...
    response, patched in at ``tau_ai.providers.openai.httpx.AsyncClient`` so
...
def _collect(monkeypatch, response: _FakeResponse) -> list:
    ...
    monkeypatch.setattr("tau_ai.providers.openai.httpx.AsyncClient", _client_factory(response))
