...
from tau_ai.client import _POOL, aclose_providers, stream_simple
...
def _reset_pool_and_client(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("tau_ai.providers.openai.httpx.AsyncClient", _RecordingClient)
    _POOL.clear()
    yield
    _POOL.clear()
...
def test_two_event_loops_do_not_share_a_client():
    ...
    # And the pool itself must not have accumulated a live entry for the first
    # (now-dead) loop — WeakKeyDictionary drops it once nothing else refs the
    # loop. CPython may or may not have collected it by this exact point (no
    # gc.collect() forced here — that would test the garbage collector, not
    # this pool), so this only asserts that at most the second loop remains.
    assert len(_POOL) <= 1
