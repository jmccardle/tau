...
"""What :func:`complete_simple` calls with each text fragment it sees.

Sync or async, because the two consumers differ: a test double counts fragments
and returns nothing, while the agent core emits an event and must be awaited.
"""
...
async def complete_simple(
    ...
    *,
    ...
    Args:
        ...
        on_text_delta: Called with each text fragment as it arrives, so a caller
            that wants the whole message can still SHOW the wait. ``None`` keeps
            the collapsed path exactly as it was — the stream is driven by
            ``result()`` alone and nothing iterates it. Sync or async; a
            coroutine is awaited before the next fragment is read, which paces
            the provider against the renderer rather than queueing deltas the
            screen never catches up with. See docs/STREAMING-SIDE-WORK.md.
    ...
    async for event in stream:
        if getattr(event, "type", None) != "text_delta":
            continue
        sunk = on_text_delta(event.delta)
