...
_LLAMACPP_SHAPE = [
    ...
    {"id": "x", "choices": [{"index": 0, "delta": {}, "finish_reason": "stop"}]},
    {
        "id": "x",
        "choices": [],
        "usage": {"prompt_tokens": 31, "completion_tokens": 80, "total_tokens": 111},
    },
]
...
def test_reasoning_content_streams_live_and_finalizes():
    ...
    final = done[0].final
    assert (
        "".join(c.thinking for c in final.content if isinstance(c, ThinkingContent))
        == "Hmm, 17*23 = 391."
    )
    assert (
        "".join(c.text for c in final.content if isinstance(c, TextContent)) == "The answer is 391."
    )
...
def test_reasoning_field_fallback_signature_is_recorded():
    ...
    final = [
        e
        for e in _run(OpenAICompletionsProvider(api_key="sk-test"), _FakeResponse(_sse(shape)))
        if isinstance(e, DoneEvent)
    ][0].final
    thinking = [c for c in final.content if isinstance(c, ThinkingContent)]
...
_REASON_THEN_TOOL_SHAPE = [
    ...
    },
    {
        ...
        "choices": [
            {
                "index": 0,
                "delta": {"tool_calls": [{"index": 0, "function": {"arguments": ' "date"}'}}]},
            }
        ],
    },
...
def test_partial_messages_carry_single_thinking_block_through_tool_call_deltas():
    ...
    events = _run(
        OpenAICompletionsProvider(api_key="sk-test"), _FakeResponse(_sse(_REASON_THEN_TOOL_SHAPE))
    )
