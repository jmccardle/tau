...
``tau-llm/tests/test_reasoning_effort.py``'s ``_CapturingClient`` /
...
Covers:
  ...
  - the ``reasoning_replay="turn"`` divergence: §7.1 threat 1. This is a
    **measured, deliberate** pi divergence (see the long comment on
    ``Model.reasoning_replay``, tau_llm/types.py:~153) — the test PINS where and
...
``AgentSession``, which tau-llm must not depend on.
"""
...
def _run_stream_chat(
    provider: OpenAICompletionsProvider, model: Model, messages: list
) -> AssistantMessage:
...
def _assert_byte_prefix(earlier: str, later: str) -> None:
    ...
    assert rest == "]" or rest.startswith(","), (
        f"unexpected continuation after the shared prefix: {rest!r}"
    )
...
def test_within_turn_stability_across_tool_call_round_trips(monkeypatch):
    ...
    messages: list[Any] = [
        UserMessage(content=[TextContent(text="please investigate")], timestamp=0)
    ]
...
def _drive_two_turn_tool_call_flow(provider: OpenAICompletionsProvider, model: Model) -> None:
    ...
    """
    _queue(
        _thinking_tool_call_reply(
            "call_1", "search", {"query": "x"}, thinking="let me think about x"
        ),
...
def test_reasoning_replay_turn_breaks_prefix_at_the_dropped_thinking_block(monkeypatch):
    ...
    (tau_llm/types.py:~153) and docs/CONSTRAINED-GEN-AND-BRANCHING-PLAN.md §7.1
